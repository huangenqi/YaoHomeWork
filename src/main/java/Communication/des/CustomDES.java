...
public class CustomDES {
    ...
    public static void main(String[] args){
        ...
        String origin="zhaooooooooooooooooooooooo";
        System.out.println("原文：\n"+origin);

        CustomDES customDES=new CustomDES("zhaohaichao",origin);
    ...
    public byte[] deal(byte[] p ,int flag){
        ...
        g_num=p_padding.length/8;//分组数
        byte[] f_p=new byte[8];
        byte[] result_data=new byte[p_padding.length];
        for(int i=0;i<g_num;i++){
            System.arraycopy(p_padding,i*8,f_p,0,8);//按组取明文到数组 f_p
